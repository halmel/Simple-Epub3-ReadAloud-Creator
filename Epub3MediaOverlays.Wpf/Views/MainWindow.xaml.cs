...
namespace Epub3MediaOverlays.Wpf
{
    public partial class MainWindow : Window
    {
        ...
        private void ReprocessButton_Click(object sender, RoutedEventArgs e)
        {
            if (sender is Button button && button.DataContext is Book book)
            {
                var reprocessWindow = new ReprocessOptionsWindow(book.Data) { Owner = this };

                if (reprocessWindow.ShowDialog() == true)
                {
                    try
                    {
                        // Delete files based on user selection
                        if (reprocessWindow.DeleteTranscriptCache)
                        {
                            string transcriptPath = book.Data.TranscriptionJsonPath;
                            if (File.Exists(transcriptPath))
                                File.Delete(transcriptPath);
                        }

                        if (reprocessWindow.DeleteAlignmentCache)
                        {
                            string alignmentPath = book.Data.WordsJsonPath;
                            string logPath = book.Data.AlignmentLogPath;
                        ...
                        if (reprocessWindow.DeleteFinalEpubFile)
                        {
                            string finalEpubPath = book.Data.FinalEpubOutputPath;
                            if (File.Exists(finalEpubPath))
                                File.Delete(finalEpubPath);
                        }

                        // Refresh book state to reflect changes
                        book.RefreshDataState();

                        // Start processing if any cache was deleted
                        if (reprocessWindow.DeleteTranscriptCache || reprocessWindow.DeleteAlignmentCache)
                        {
                            AlignButton_Click(sender, e);
                        }
                        else if (reprocessWindow.DeleteFinalEpubFile)
                        {
                            MessageBox.Show("Final EPUB deleted. You can now align the book again to regenerate it.", 
                                "Reprocess Complete", MessageBoxButton.OK, MessageBoxImage.Information);
                        }
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show($"Failed to reprocess book:\n{ex.Message}", "Error",
        ...
        private void ToggleAudioDetails_Click(object sender, RoutedEventArgs e)
        {
            if (sender is Button button && button.DataContext is Book book)
            {
                book.IsAudioDetailsExpanded = !book.IsAudioDetailsExpanded;
            }
        }
